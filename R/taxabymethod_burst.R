# This code creates the starburst figure by taxa by method in the hotspot manuscript
library(tidyverse)
library(cowplot)
...
# Make dataframe seprating taxa and methods into rows and condensing taxa into broader categories
methods_df <- main %>% 
...
empty_bar <- 1
to_add <- data.frame(matrix(NA,empty_bar*nlevels(methods_df$Methods),ncol(methods_df)))
colnames(to_add) <- colnames(methods_df)
to_add$Methods <- rep(levels(methods_df$Methods), each = empty_bar)
...
p1 <- ggplot(methods_df, aes(x=Taxa2, y=n, fill=Methods)) +       
  ...
  coord_polar() + 
  geom_text(data=label_data, aes(x=id, y=n+10, label=Taxa2, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=4, angle=label_data$angle, inherit.aes = FALSE ) +
  
  # Add base line information
  geom_segment(data=base_data, aes(x = start, y = -5, xend = end, yend = -5), colour = "black", alpha=0.8, linewidth=0.6 , inherit.aes = FALSE )  +
  geom_text(data=base_data, aes(x = title, y = -17, label = Methods, hjust = base_data$hjust), angle = base_data$angle, 
