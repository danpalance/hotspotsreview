...
empty_bar <- 1
to_add <- data.frame(matrix(NA,empty_bar*nlevels(realm_df$Type), ncol(realm_df)))
colnames(to_add) <- colnames(realm_df)
to_add$Type <- rep(levels(realm_df$Type), each = empty_bar)
realm_df <- rbind(realm_df, to_add)
realm_df$Type<- as.factor(realm_df$Type)
realm_df <- as.data.frame(realm_df)
realm_df <- realm_df %>% arrange(Type,-n) # Sort by Methods and then number of obs so the bar graphs are ordered from highest to smallest
