...
# problems <- hs_drivers %>% 
#   filter(Drivers_examined_condensed %in% c("fishing","human activity"),
#          Category == "Biophysical")

saveRDS(hs_drivers, file ="output/hs_drivers.rds")
...
drivers_df <- hs_drivers %>% 
  ...
  mutate(Total=sum(n)) %>% 
  ungroup() %>% 
  mutate(Drivers = as.factor(Drivers))


# Setup empty bar spacers between realms
empty_bar <- 1
to_add <- data.frame(matrix(NA,empty_bar*nlevels(drivers_df$Drivers),ncol(drivers_df)))
colnames(to_add) <- colnames(drivers_df)
to_add$Drivers <- rep(levels(drivers_df$Drivers), each = empty_bar)
drivers_df <- rbind(drivers_df, to_add)
drivers_df$Drivers <- as.factor(drivers_df$Drivers)
drivers_df <- as.data.frame(drivers_df)
drivers_df <- drivers_df %>% arrange(Drivers,-n) # Sort by Type and then number of obs so the bar graphs are ordered from highest to smallest
drivers_df$id <- seq(1, nrow(drivers_df))

# Get the name and the y position of each label
label_data <- drivers_df
...
# could add another Type category called label to trick it into thinking there are labels there and then use annotate to put the text in
# Assemble graph 
# Make the plot 
p1 <- ggplot(drivers_df, aes(x=Drivers, y=n, fill=Category)) +       
  ...
  annotate("text", x = rep(max(drivers_df$id),5), y = c(0,5, 10, 15, 20), label = c("0","5", "10", "15", "20") , color="grey", size=3 , angle=0, fontface="bold", hjust=1) +
  ylim(-100,120) +
  theme_minimal() +
  ...
  coord_polar() + 
  geom_text(data=label_data, aes(x=id, y=n+10, label=Type, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=3.5, angle=label_data$angle, inherit.aes = FALSE ) +
  
  # Add base line information
  geom_segment(data=base_data, aes(x = start, y = -5, xend = end, yend = -5), colour = "black", alpha=0.8, linewidth=0.6 , inherit.aes = FALSE )  +
  geom_text(data=base_data, aes(x = title, y = -17, label=Drivers, hjust=hjust), angle = base_data$angle,
            colour = "black", alpha=0.8, size=3.5, fontface="bold", inherit.aes = FALSE)

p2 <- ggplot(drivers_df) +       
  geom_col(aes(x=as.factor(id), y=Total, fill=Drivers), col=NA,width=1.5) + 
  #scale_fill_manual(values=c("Anthropogenic"="#CD950C","Biophysical"="#0000CD","Ecoimpact"="#228B22")) +
  coord_polar() +
  theme_minimal() +
  theme(legend.position = "none",
...
# Nest the circular bar charts with total Type inside Type by realm
ggdraw() +
  draw_plot(p2,scale=0.38) +
  draw_plot(p1,scale=1)
ggsave(file ="Figs/typebydrivers_burst_blank.png", scale = 2.5)
