...
hsr_df$Category <- fct_collapse(hsr_df$Type, Biophysical=c("Foraging","Habitat","Nutrients & Biogeochemical-Cycling",
                                ...
                                Anthropogenic = c("Pollution","Warming"),
                                Ecoimpact = c("Invasives", "Mortality","Acidification","Fisheries & Bycatch","Multi-Risk & Threat","Socio-ecological",
                                              "Bioaccumulation","Metabolic Production"))
...
meow <- readOGR("Hotspot Review/Marine Ecoregions/",layer="meow_ecos") # read in shapefile
meow_realm <- tidy(meow, region = "REALM") # extract a dataframe of the realms from the shape file
meow_realm$id <- as.factor(meow_realm$id) # Convert the id columns to factor for plotting

# Extract how many hotspots in a given realm #
meow_shp <- read_sf("Hotspot Review/Marine Ecoregions/",layer="meow_ecos") # read in meow as a shapefile using sf package
...
meow_poly <- readOGR("Hotspot Review/Marine Ecoregions/",layer="meow_ecos")
...
saveRDS(meow_hs, file ="Hotspot Review/Data/meow_hs.RDS")

# Insert the studies back into the dataframe now that they all have a realm assigned or NA for realm
outside_meow <- anti_join(hsr_df,meow_hs,by="Title") %>% 
  mutate(REALM = "not applicable") %>% 
  select(-c(Lat,Lon)) 

# Combine meow_hs (spatial df) and the studies outside of eco realms to create a master dataframe for non spatial analysis, note there are 5 studies that spread across mutliple
# ecoregions so there will be a few more rows than the HSR_DF (294 vs 299)
...
saveRDS(master, file ="Hotspot Review/Data/master.RDS")

# FIGURE 1: Create hotspot map of # of studies by realm and add density plots of # of studies by lat and lon on sides ####
heatmap_realm <- ggplot() +
  #geom_polygon(data=meow_realm,aes(x=long, y=lat,group=group),fill="blue") +
  #geom_polygon(data=heatmap_realm_data, aes(x=long, y=lat,group = group,fill=n),color="black") +
  #scale_fill_distiller(bquote(~bold("# of Studies")),
  #                     palette = "Spectral",limits = c(0, max(heatmap_realm_data$n))) +
  geom_sf(data=world, color="black",fill="wheat2") +
  geom_point(data=hs_locations, aes(x=Lon,y=Lat), color="black", fill="blue", alpha=0.5, pch=21,size=3) +
  theme_classic() +
  ...
  geom_point(data=hs_locations, aes(x=Lon,y=Lat), color="black", fill="black", alpha=0, pch=21,size=2) 

heatmap_realm

# Add PDF plots of lat lon study distribution
heatmap_realm_dens <- ggMarginal(heatmap_realm,fill = "blue",alpha=0.5,adjust=0.45,size=12)
ggsave(plot=heatmap_realm_dens,
       "~/Documents/UCSC/Dissertation/Hot Spots Review Paper/Hotspot Manuscript/Hotspot Review/Figures/MeowWorldDist_GradientREALM.png",scale=2)

# Create dataframe of methods grouped by hotspot type, need to add back in the studies that don't fall within a marine ecoregion
# Create spatial dataframe of study methods by eco realm
methods_spatial <- meow_hs %>% 
  separate_rows(Methods, sep=", ") %>% 
  group_by(Year,PROVINCE,REALM,ECOREGION) %>% 
  count(Methods)

# Create dataframe that sums number of studies by method type
methods_total <- master %>% 
  separate_rows(Methods, sep=", ") %>% 
  count(Methods)

# Create dataframe where multiple method types were used
methods_mutli <- hsr_df %>% 
...
# Create dataframe where just one method type was used
methods_single <- hsr_df %>% 
  mutate("Paper" = paste0(Authors, ", ",Year)) %>%
  separate_rows(Methods, sep=", ") %>% 
  count(Paper) %>% 
  separate(Paper, into = c('Author', 'Year'), sep = ", ") %>% 
...
#ggsave(file ="~/Documents/UCSC/Dissertation/Hot Spots Review Paper/Hotspot Manuscript/Hotspot Review/Figures/Studies_Methods.png")  

# Stacked bar chart of methods by realm
ggplot()+
  geom_bar(data=methods_spatial,aes(x=reorder(Methods,n,sum),y=n,fill=REALM), stat="identity")+
  theme_classic()+
  scale_y_continuous(expand=c(0,0))+
  labs(x="Method",y="# of Studies") +
  theme(axis.text.x = element_text(angle = 90,hjust=1,vjust = 0.5))+
  scale_fill_brewer(palette = "Paired")
#ggsave(file ="~/Documents/UCSC/Dissertation/Hot Spots Review Paper/Hotspot Manuscript/Hotspot Review/Figures/MethodsRealm.png")

# Create stacked bar chart of hotspot type by realm
type_spatial <- meow_hs %>% 
  group_by(Year,PROVINCE,REALM,ECOREGION) %>% 
  count(Type)
...
taxa_spatial <- meow_hs %>% 
  separate_rows(Taxa, sep=",") %>% 
  group_by(Authors,PROVINCE,REALM,ECOREGION) %>% 
  count(Taxa)
...
taxa_df$Taxa <- factor(taxa_df$Taxa , levels=c("1"="microbes","2"="microalgae","3"="macroalgae","4"="inverts","5"="sponges","6"="coral","7"="urchins","8"="seastars",
                                                "9"="crustaceans","10"="krill","11"="mollusks","12"="plankton","13"="cart fish","14"="bony fish","15"="reef fish","16"="plants",
                                                "17"="sea turtles","18"="sea snake","19"="seabirds","20"="sirenians","21"="pinnipeds","22"="fissipeds","23"="cetaceans","24"="jaguars"))
...
#taxa_df <- taxa_df %>% fct_reorder(Taxa,Order)
...
p1 <- ggplot(taxa_df, aes(x=REALM, y=n, fill=Taxa)) +       
  ...
  annotate("text", x = rep(max(taxa_df$id)+0.3,5), y = c(0,5, 10, 15, 20), label = c("0","5", "10", "15", "20") , color="grey", size=3 , angle=0, fontface="bold", hjust=1) +
  ylim(-100,120) +
  theme_minimal() +
...
##### Create Figure 4 - The timeline of methods evolution ######
